...
@app.post('/getRanking')
def get_ranking_api():
    distances_folder_path = request.args.get('distancesPath')
    k = int(request.args.get('k'))
    dataset = request.args.get('queryDataset')
    attribute = request.args.get('queryAttribute')
    model = joblib.load('predictive_model.pkl')

    return get_ranking(distances_folder_path, k, dataset, attribute, model).to_dict(orient="records")
